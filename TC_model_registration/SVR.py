...
def get_registration_parameters(error_in):
    norm_err = error_in / 100
    # Alpha law
    if norm_err <= 0.68:
        a = 1.63e15 * np.exp(-((norm_err - 3.4215) / 0.4731) ** 2) + 2.9892 * np.exp(-((norm_err - 0.2184) / 0.1818) ** 2)
    else:
        a = np.exp(-45.77 * norm_err + 31.376) + 3
    # Beta law
    b = 0.5143 * np.exp(-((norm_err - 0.5872) / 0.0406) ** 2) + 0.2755 * np.exp(-((norm_err - 0.327) / 0.0506) ** 2) + 0.04
    return a, b
...
filename_out = r"C:\Users\olocc\OneDrive\Desktop\Thesis_new\TC_model_registration\Results\SVR.txt"
...
for label in labels:
    for num_config in range(num_configs):
        ...
        a, b = get_registration_parameters(error_in)
        print("SVR registration parameters:")
        print("Alpha =", a)
        print("Beta =", b)
        starting_time = time.time()
        # SVR registration
        tf_param = l2dist_regs.registration_svr(
            np.asarray(src_norm.points), np.asarray(tgt_norm.points), tf_type_name="nonrigid",
            maxiter=2, alpha=a, beta=b
        )
        reg_pts = tf_param.transform(np.asarray(src_norm.points))
        reg_pts = denormalize_points(reg_pts, tgt_center, tgt_factor)
        src_def_down = o3d.geometry.PointCloud()
        src_def_down.points = o3d.utility.Vector3dVector(reg_pts)
        ...
        # show_point_clouds(src_def, tgt, "After SVR registration - Separated clouds")
