...
scale = 680
# labels related to deformation degree
...
def compute_parameters(error_in, src, den_params):
    ...
    norm_err = error_in / 100
    # Compute parameters
    alpha = (-0.9867 * norm_err ** 3 + 1.4363 * norm_err ** 2 + 0.1229 * norm_err + 0.4334) * mean_dist
    beta = (58.45 * np.exp(-((norm_err - 0.004953) /  0.02732) ** 2) + 1.872 * np.exp(-((norm_err - 0.1501) / 0.04947) ** 2) + 1.055 * np.exp(-((norm_err - 0.2747) / 0.2315) ** 2)) * mean_dist
    delta = 0.9 - 0.5 * norm_err
    sigma = 0.3
    print("Mean distance:", mean_dist)
    print("GMMReg parameters:")
    print("Beta =", beta)
    print("Alpha =", alpha)
    print("Sigma =", sigma)
    print("Delta =", delta)
...
# Extend deformation to all points of the original cloud
...
filename_out = r"C:\Users\olocc\OneDrive\Desktop\Thesis_new\TC_model_registration\Results\GMMReg.txt"
...
for label in labels:
    for num_config in range(num_configs):
        ...
        tgt = o3d.io.read_point_cloud(tgt_filename)
        show_point_clouds(src, tgt, "Before Registration")
        ...
        # Rigid pre-alignement for high-deformed clouds
        if error_in > 60.0:
        ...
        a, b, d, s = compute_parameters(error_in, src_norm, [src_center, src_factor])
        # GMMReg registration
        tf_param = l2dist_regs.registration_gmmreg(
            np.asarray(src_norm.points), np.asarray(tgt_norm.points),
            tf_type_name="nonrigid", alpha = a, beta = b, delta = d, sigma = s
        )
        reg_pts = tf_param.transform(np.asarray(src_norm.points))
        src_def_down = o3d.geometry.PointCloud()
        src_def_down.points = o3d.utility.Vector3dVector(
            denormalize_points(reg_pts, tgt_center, tgt_factor)
        )
        ...
        # src_def.translate(np.array([0, 0, 0.2 * scale]))
        # show_point_clouds(src_def, tgt, "After GMMReg registration - Separated clouds")
